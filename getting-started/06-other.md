...
Although the previous usage patterns have used the Structurizr DSL, Structurizr workspaces can also be created using a number of compatible tools:

1. The [Structurizr DSL](/dsl) (a text-based DSL).
2. [Structurizr for Java](/java).
3. Community tools for [.NET, Python, PHP, TypeScript, etc](/community#authoring-tools).

The recommended approach for most teams is to use the Structurizr DSL - it results in more concise workspace definitions than using a programming language, and it's generally easier for a wider audience to understand.
...
Irrespective of which tool you use, the output is a workspace that can be serialised to a JSON document
via an [open JSON data format](/openapi). This JSON version of the workspace can be pushed to a Structurizr server, rendered with local, or exported to PlantUML/Mermaid.

## Manual creation vs reverse-engineering

The typical approach taken by teams is to manually create their workspace. For example, if you're using the Structurizr DSL, you are explicitly defining all people, software systems, containers, components, etc using the DSL language. This can be quick and effective, but requires that the DSL definition is changed, and kept up to date, whenever the software architecture changes.
